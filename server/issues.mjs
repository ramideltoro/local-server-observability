...
export const issueId = (system, check, signature) =>
  createHash("sha256")
    .update(JSON.stringify([system, check, signature]))
    .digest("hex")
    .slice(0, 24);
...
export function reconcile(previous, checks, at) {
  ...
  for (const check of checks) {
    const existing = issues.filter(
      (i) => i.system === check.system && i.check === check.id,
    );
    ...
    for (const failure of failures) {
      ...
      issue.status = status;
      issue.lastSeen = at;
      issue.occurrences += failure.count || 1;
      issue.severity = failure.severity || "warning";
      issue.summary = failure.summary;
      issue.evidence = failure.evidence || [];
      issue.evidenceAt = at;
    ...
    for (const issue of existing) {
      if (
        failures.some(
          (f) => issueId(check.system, check.id, f.signature) === issue.id,
        ) ||
        issue.status === "Remediated"
      )
        continue;
  ...
  for (const issue of issues)
    if (Date.parse(at) - Date.parse(issue.evidenceAt || at) > 90 * 86400000)
