...
export function workspace({ owner, grafana, json, cached, root }) {
  ...
  return async (req, res, u) => {
    if (
      ...
      const now = Date.now();
      let r = rates.get(ip);
      if (!r || r.until < now) r = { count: 0, until: now + 60000 };
      r.count++;
      rates.set(ip, r);
      if (rates.size > 2000) rates.delete(rates.keys().next().value);
      if (r.count > 240) {
        json(res, 429, { error: "Request budget exceeded" });
        return true;
      }
    }
    const p = u.pathname;
    if (p.startsWith("/grafana/") || p.startsWith("/owner/grafana/")) {
      ...
      await proxyGrafana(req, res, u, privateView);
      return true;
    }
    const routes = [
    ...
    if (p === "/api/owner/scan" || p === "/api/owner/publish") {
      ...
      if (
        ...
        json(
          res,
          403,
          { error: "Owner and same-origin request required" },
          true,
        );
        return true;
      }
      if (!process.env.AUTOMATION_TOKEN) {
        json(res, 503, { error: "Manual dispatch is not configured" }, true);
        return true;
      }
      if (p === "/api/owner/publish") {
        const input = JSON.parse((await body(req, 2048)).toString());
        json(
          res,
          201,
          await publishDashboard(input.uid, process.env.AUTOMATION_TOKEN),
          true,
        );
        return true;
      }
      const response = await fetch(
        "https://api.github.com/repos/ramideltoro/local-server-infra/actions/workflows/inspect.yml/dispatches",
        {
          method: "POST",
          headers: {
      ...
      json(
        ...
        {
          message: response.ok
            ? "Inspection queued in GitHub Actions"
            : "Dispatch failed",
        },
        true,
      );
    ...
    if (p.endsWith("/reports")) {
      ...
      json(res, 200, {
        ...
        overdue:
          !s.lastSuccess ||
          Date.now() - Date.parse(s.lastSuccess) > 27 * 3600000,
      });
    }
    if (p.endsWith("/rules"))
      json(
